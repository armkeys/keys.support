...
var show_add_level_modal      = function( e ) {
	...
	$("#" + add_level_modal.data.id).find(".wlm-levels-select").allow_select_all();
	reset_calendar();
}
...
var show_delete_level_modal     = function( e ) {
	...
	$( "#" + delete_level_modal.data.id ).find( ".wlm-levels" ).allow_select_all();
	reset_calendar();
}
			...
			if ( result != 0 || result != "" ) {
				try {
					...
					if ( result_data.success ) {
						...
						$body.find( '.wlm-datetimepicker' ).daterangepicker(
							{
								singleDatePicker: true,
								timePicker: true,
								showCustomRangeLabel: false,
			...
			on_success: function( $me, $result) {
				if ( $result.success && $result.data ) {
					...
					if ( $result.level_data ) {
							...
							function( userid, level_data ) {
								$.each(
									...
									function( levelid, data ) {
										...
										$markup    += '<td class="col-3"><input type="text" class="form-control wlm-datetimepicker" value="' + data.regdate + '" name="lvltime[' + levelid + ']"  placeholder="Registration Date" /></td>';
						...
						$holder.find( '.wlm-datetimepicker' ).daterangepicker(
							{
								singleDatePicker: true,
								timePicker: true,
								showCustomRangeLabel: false,
