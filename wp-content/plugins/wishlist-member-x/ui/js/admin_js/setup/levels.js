...
wlm3_screen_levels.prototype = {
	...
	list_levels: function () {
		...
		$('#levels-list-table td.level-order').each( function( i ) {
			i++;
			$( this ).text( i );
		});
	...
	init: function () {
			...
			stop: function (event, ui) {
				...
				$.post(WLM3VARS.ajaxurl, data, function (result) {
					$.each(data.reorder, function (i, o) {
						...
						$('#levels-list-table tbody > tr[data-id=' + i + '] > td.level-order').text(o);
					});
					js_levels = [];
		...
		$('#levels-create')
			...
			.on('click.wlm3levels', '.create-form-button', { that: this }, function (e) {
				...
				$('<div/>').save_settings({
					data: data, on_done: function (me, data, jqXHR, textStatus) {
