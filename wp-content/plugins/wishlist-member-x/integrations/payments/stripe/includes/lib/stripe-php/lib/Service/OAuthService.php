...
class OAuthService extends \WishListMember\PaymentProviders\Stripe\PHPLib\Service\AbstractService
{
    /**
     ...
     *
     * @return \WishListMember\PaymentProviders\Stripe\PHPLib\StripeObject the object returned by Stripe's Connect API
     */
    ...
    public function authorizeUrl($params = null, $opts = null)
    {
        ...
        $query = \WishListMember\PaymentProviders\Stripe\PHPLib\Util\Util::encodeParameters($params);
     ...
     * @param null|array $opts
     *
     * @throws \WishListMember\PaymentProviders\Stripe\PHPLib\Exception\OAuth\OAuthErrorException if the request fails
     *
    ...
    private function _getClientId($params = null)
    {
        ...
        if (null === $clientId) {
            $msg = 'No client_id provided. (HINT: set your client_id using '
              . '`new \WishListMember\PaymentProviders\Stripe\PHPLib\StripeClient([clientId => <CLIENT-ID>
     ...
     *
     * @return \WishListMember\PaymentProviders\Stripe\PHPLib\Util\RequestOptions
     */
    private function _parseOpts($opts)
    {
        if (\is_array($opts)) {
            if (\array_key_exists('connect_base', $opts)) {
                ...
                // \WishListMember\PaymentProviders\Stripe\PHPLib\Service\OAuthService from \WishListMember\PaymentProviders\Stripe\PHPLib\OAuth, where `connect_base`
                // Was the name of the parameter that behaves as `api_base` does here.
                throw new \WishListMember\PaymentProviders\Stripe\PHPLib\Exception\InvalidArgumentException('Use `api_base`, not `connect_base`');
            }
        }

        return \WishListMember\PaymentProviders\Stripe\PHPLib\Util\RequestOptions::parse($opts);
    }
