...
```php
$stripe = new \WishListMember\PaymentProviders\Stripe\PHPLib\StripeClient('sk_test_BQokikJOvBiI2HlWgH4olfQ2');
...
$curl = new \WishListMember\PaymentProviders\Stripe\PHPLib\HttpClient\CurlClient();
$curl->setTimeout(10); // default is \WishListMember\PaymentProviders\Stripe\PHPLib\HttpClient\CurlClient::DEFAULT_TIMEOUT
$curl->setConnectTimeout(5); // default is \WishListMember\PaymentProviders\Stripe\PHPLib\HttpClient\CurlClient::DEFAULT_CONNECT_TIMEOUT

echo $curl->getTimeout(); // 10
echo $curl->getConnectTimeout(); // 5

// tell Stripe to use the tweaked client
\WishListMember\PaymentProviders\Stripe\PHPLib\ApiRequestor::setHttpClient($curl);
...
```php
// set up your tweaked Curl client
$curl = new \WishListMember\PaymentProviders\Stripe\PHPLib\HttpClient\CurlClient([CURLOPT_PROXY => 'proxy.local:80']);
// tell Stripe to use the tweaked client
\WishListMember\PaymentProviders\Stripe\PHPLib\ApiRequestor::setHttpClient($curl);
```
...
```php
\WishListMember\PaymentProviders\Stripe\PHPLib\WLM_Stripe::setLogger($logger);
```
...
```php
$curl = new \WishListMember\PaymentProviders\Stripe\PHPLib\HttpClient\CurlClient([CURLOPT_SSLVERSION => CURL_SSLVERSION_TLSv1]);
\WishListMember\PaymentProviders\Stripe\PHPLib\ApiRequestor::setHttpClient($curl);
```
...
```php
\WishListMember\PaymentProviders\Stripe\PHPLib\WLM_Stripe::setCABundlePath("path/to/ca/bundle");
```
...
```php
\WishListMember\PaymentProviders\Stripe\PHPLib\WLM_Stripe::setMaxNetworkRetries(2);
```
...
```php
\WishListMember\PaymentProviders\Stripe\PHPLib\WLM_Stripe::setEnableTelemetry(false);
```
...
```php
\WishListMember\PaymentProviders\Stripe\PHPLib\WLM_Stripe::setAppInfo("MyAwesomePlugin", "1.2.34", "https://myawesomeplugin.info");
```
