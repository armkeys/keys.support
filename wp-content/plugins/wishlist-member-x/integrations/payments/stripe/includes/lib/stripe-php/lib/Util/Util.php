...
use WishListMember\PaymentProviders\Stripe\PHPLib\StripeObject;
...
abstract class Util
{
    ...
    {
        $types = \WishListMember\PaymentProviders\Stripe\PHPLib\Util\ObjectTypes::MAPPING;
        if (self::isList($resp)) {
            $mapped = [];
            foreach ($resp as $i) {
        ...
        if (\is_array($resp)) {
            if (isset($resp['object']) && \is_string($resp['object']) && isset($types[$resp['object']])) {
                ...
                $class = \WishListMember\PaymentProviders\Stripe\PHPLib\StripeObject::class;
            }
    ...
    {
        if ($h instanceof \WishListMember\PaymentProviders\Stripe\PHPLib\ApiResource) {
            return $h->id;
        }
        if (static::isList($h)) {
            $results = [];
