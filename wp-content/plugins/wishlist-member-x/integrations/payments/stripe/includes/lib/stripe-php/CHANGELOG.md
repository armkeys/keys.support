...
## 7.25.0 - 2020-02-14
* [#879](https://github.com/stripe/stripe-php/pull/879) Make `\WishListMember\PaymentProviders\Stripe\PHPLib\Collection` implement `\Countable`
...
-   Use `\WishListMember\PaymentProviders\Stripe\PHPLib\<class>::class` constant instead of strings ([#643](https://github.com/stripe/stripe-php/pull/643))
...
-   [#539](https://github.com/stripe/stripe-php/pull/539) Fix methods on `\WishListMember\PaymentProviders\Stripe\PHPLib\PaymentIntent` to properly pass arguments to the API.

## 6.19.4 - 2018-10-11

-   [#534](https://github.com/stripe/stripe-php/pull/534) Fix PSR-4 autoloading for `\WishListMember\PaymentProviders\Stripe\PHPLib\FileUpload` class alias

## 6.19.3 - 2018-10-09

-   [#530](https://github.com/stripe/stripe-php/pull/530) Add constants for `flow` (`FLOW_*`), `status` (`STATUS_*`) and `usage` (`USAGE_*`) on `\WishListMember\PaymentProviders\Stripe\PHPLib\Source`
...
-   [#520](https://github.com/stripe/stripe-php/pull/520) Rename `\WishListMember\PaymentProviders\Stripe\PHPLib\FileUpload` to `\WishListMember\PaymentProviders\Stripe\PHPLib\File`
...
-   [#502](https://github.com/stripe/stripe-php/pull/502) Add `isDeleted()` method to `\WishListMember\PaymentProviders\Stripe\PHPLib\StripeObject`

## 6.12.0 - 2018-07-28

-   [#501](https://github.com/stripe/stripe-php/pull/501) Add support for scheduled query runs (`\WishListMember\PaymentProviders\Stripe\PHPLib\Sigma\ScheduledQueryRun`) for Sigma
...
-   [#498](https://github.com/stripe/stripe-php/pull/498) Internal improvements to the `\WishListMember\PaymentProviders\Stripe\PHPLib\ApiResource.classUrl()` method
...
-   [#493](https://github.com/stripe/stripe-php/pull/493) Add PHPDoc for `auto_advance` on `\WishListMember\PaymentProviders\Stripe\PHPLib\Invoice`
...
-   [#481](https://github.com/stripe/stripe-php/pull/481) Add new `\WishListMember\PaymentProviders\Stripe\PHPLib\Discount` and `\WishListMember\PaymentProviders\Stripe\PHPLib\OrderItem` classes, add more PHPDoc describing object attributes
...
-   [#479](https://github.com/stripe/stripe-php/pull/479) Fix unnecessary traits on `\WishListMember\PaymentProviders\Stripe\PHPLib\InvoiceLineItem`

## 6.7.2 - 2018-05-28

-   [#471](https://github.com/stripe/stripe-php/pull/471) Add `OBJECT_NAME` constant to all API resource classes, add `\WishListMember\PaymentProviders\Stripe\PHPLib\InvoiceLineItem` class
...
-   [#458](https://github.com/stripe/stripe-php/pull/458) Add PHPDoc for `account` on `\WishListMember\PaymentProviders\Stripe\PHPLib\Event`
...
-   [#453](https://github.com/stripe/stripe-php/pull/453) Add constants for `reason` (`REASON_*`) and `status` (`STATUS_*`) on `\WishListMember\PaymentProviders\Stripe\PHPLib\Dispute`

## 6.3.2 - 2018-02-27

-   [#452](https://github.com/stripe/stripe-php/pull/452) Add PHPDoc for `amount_paid` and `amount_remaining` on `\WishListMember\PaymentProviders\Stripe\PHPLib\Invoice`

## 6.3.1 - 2018-02-26

-   [#443](https://github.com/stripe/stripe-php/pull/443) Add event types as constants to `\WishListMember\PaymentProviders\Stripe\PHPLib\Event` class
...
-   [#442](https://github.com/stripe/stripe-php/pull/442) Fix PHPDoc for `\WishListMember\PaymentProviders\Stripe\PHPLib\Error\SignatureVerification`
...
*   `\WishListMember\PaymentProviders\Stripe\PHPLib\AttachedObject` no longer exists. Attributes that used to be instances of `\WishListMember\PaymentProviders\Stripe\PHPLib\AttachedObject` (such as `metadata`) are now instances of `\WishListMember\PaymentProviders\Stripe\PHPLib\StripeObject`.

-   Attributes that used to be PHP arrays (such as `legal_entity->additional_owners` on `\WishListMember\PaymentProviders\Stripe\PHPLib\Account` instances) are now instances of `\WishListMember\PaymentProviders\Stripe\PHPLib\StripeObject`, except when they are empty. `\WishListMember\PaymentProviders\Stripe\PHPLib\StripeObject` has array semantics so this should not be an issue unless you are actively checking types.

*   `\WishListMember\PaymentProviders\Stripe\PHPLib\Collection` now derives from `\WishListMember\PaymentProviders\Stripe\PHPLib\StripeObject` rather than from `\WishListMember\PaymentProviders\Stripe\PHPLib\ApiResource`.
...
-   Add `\WishListMember\PaymentProviders\Stripe\PHPLib\WLM_Stripe::setLogger()` to support an external PSR-3 compatible logger
...
-   Rename `\WishListMember\PaymentProviders\Stripe\PHPLib\Object` to `\WishListMember\PaymentProviders\Stripe\PHPLib\StripeObject` (PHP 7 compatibility)
...
-   Switched to Stripe namespace instead of WishListMember\PaymentProviders\Stripe\PHPLib\_ class name prefiexes (thanks @chadicus!)
