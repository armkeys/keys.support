    ...
    <div class="body">
        <div class="row">
            <template class="wlm3-form-group">
                {
                    label : '<?php echo esc_js(__('Autoresponder Email', 'wishlist-member')); ?>',
                    type : 'text',
                    name : 'email[<?php echo esc_attr($level->id); ?>]',
                    column : 'col-12',
                    'data-mirror-value' : '#getresponselegacy-email-<?php echo esc_attr($level->id); ?>',
                    tooltip : '<?php echo esc_js(__('Create an auto responder email list in GetResponse and paste that email address into the Autoresponse Email field.', 'wishlist-member')); ?>',
                }
            </template>
            <template class="wlm3-form-group">
                {
                    label : '<?php echo esc_js(__('Unsubscribe Email', 'wishlist-member')); ?>',
                    type  : 'text',
                    name  : 'remove[<?php echo esc_attr($level->id); ?>]',
                    column : 'col-12',
                    'data-mirror-value' : '#getresponselegacy-remove-<?php echo esc_attr($level->id); ?>',
                    tooltip : '<?php echo esc_js(__('Create an Unsubscribe email list in GetResponse and paste that email address into the Autoresponse Email field.', 'wishlist-member')); ?>',
                }
            </template>
        </div>
    </div>
