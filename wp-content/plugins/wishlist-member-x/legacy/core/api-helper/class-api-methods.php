...
if (! class_exists('WLMAPIMethods')) {
    ...
    class WLMAPIMethods
    {
         ...
         *
         * @param integer $level_id - ID of the level to get terms for
         *
        ...
        public function get_member_by($field, $value, $raw = false)
        {
            ...
            if ($this->internal && function_exists('get_user_by')) {
                ...
                if (! empty($data)) {
                    $response = [
                        'success'         => 1,
                        'member'          => wlmapi_get_member($data->ID),
                        'members'         => [
                            'member' => [
                                [
                                    'id'         => $data->ID,
        ...
        public function get_member_levels($user_id, $raw = false)
        {
            ...
            if ($this->internal && function_exists('WishListMemberAPIRequest')) {
                $response = WishListMemberAPIRequest('/members/' . $user_id);
                $response = wlm_or(wlm_arrval($response, 'member', 0, 'Levels'), []);
                return $response;
            }
         ...
         *
         * @param integer $user_id     - The ID of the user to check againts
