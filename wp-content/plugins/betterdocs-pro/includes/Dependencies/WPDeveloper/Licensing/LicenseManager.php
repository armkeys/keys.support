...
class LicenseManager {
	 ...
	 * @throws Exception
	 */
	public static function get_instance( $args ): LicenseManager {
	...
	public function admin_notices(): void {
		...
		$message = sprintf( __( '%1$sWarning:%2$s Your website is currently vulnerable. %3$sActivate your %5$s License Key%4$s to receive regular updates and secure your WordPress website.' ), '<strong>', '</strong>', '<a style="text-decoration: underline; font-weight: bold;" href="' . admin_url( 'admin.php?page=' . $this->page_slug ) . '">', '</a>', $this->item_name );
	...
	public function hide_license_key( $_license ): string {
	...
	public function set_license(): bool {
	...
	public function set_license_data( $response, $expiration = null ): void {
		if ( null === $expiration ) {
	...
	public function remove_license_data(): bool {
	...
	private function remove_error(): void {
	...
	public function set_status( $status = 'valid' ): bool {
		return update_option( "{$this->db_prefix}_license_status", $status, 'no' );
	}

	public function removeData( $withError = true ): void {
	...
	public function addData( $response ): void {
		$this->set_license();
	...
	private function delete( $key ): bool {
		$option = "{$this->db_prefix}_{$key}";
